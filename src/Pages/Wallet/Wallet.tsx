  ...
  return (
    <div className="min-h-screen overflow-x-auto bg-gray-100 dark:bg-black dark:text-white padding-x">
      ...
      <header className="sm:px-20 px-2 flex justify-between py-6 items-center w-full bg-white dark:bg-black">
        ...
        <div className="flex items-center sm:gap-4 gap-2">
          {loading ? (
            ...
            <h2 className="sm:text-[20px] text-[12px] max-s8:text-[10px] font-sans font-medium  leading-text tracking-text dark:text-white text-slate-gray cursor-pointer">{data?.paymentLink?.title}</h2>
          )}
          <Button
            size="icon"
            variant="ghost"
      ...
      <div className="flex md:w-4/5 mx-auto flex-col justify-center items-center gap-10 dark:bg-background-paymentDark bg-background-payment text-white text-center py-20 rounded-[15px] mb-10">
        ...
        <div className="flex flex-col gap-2">
          {loading ? (
            ...
            <h2 className="text-[40px] font-calsans leading-text tracking-text font-bold">$ {data?.paymentLink?.totalAmountReceived
            }</h2>
          )}
      ...
      <div className="p-6 overflow-x-auto bg-[#F5F7FA] dark:bg-[#191B1F] rounded-[15px] dark:rounded-[20px]">
        <div className="flex md:w-4/5 mx-auto max-md:flex-col-reverse gap-5 justify-between items-center">
          ...
          {loading ? (
            Array.from({ length: 5 }).map((_, idx) => (
              ...
              >
                ...
                <div className="p-3"><Skeleton className="h-4 w-32" /></div>
                <div className="p-3"><Skeleton className="h-4 w-18" /></div>
              </div>
            ))
