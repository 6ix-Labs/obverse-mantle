...
const PaymentStep: React.FC<PaymentStepProps> = ({ step, currentStep, title, description, status }) => {
  ...
  return (
    <div className="flex gap-3 items-start">
      <div className={`flex justify-center items-center w-8 h-8 text-white rounded-full ${getBgColor()}`}>
        {getStepIcon()}
      </div>
...
const WalletButton: React.FC<WalletButtonProps> = ({
  ...
  if (isConnected && walletAddress) {
    return (
      <div className="flex gap-2 items-center">
        <div className="flex gap-2 items-center px-3 py-2 bg-green-100 rounded-lg dark:bg-green-900/30">
          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
          <span className="text-sm text-green-700 dark:text-green-400">{truncateAddress(walletAddress)}</span>
          <button
            onClick={copyToClipboard}
            className="p-1 ml-1 rounded transition-colors hover:bg-green-200 dark:hover:bg-green-800/50"
        ...
        <button onClick={onDisconnect} className="text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
          Disconnect
        </button>
      </div>
    );
  ...
  return (
    ...
    >
      {isLoading ? <FiLoader className="w-4 h-4 animate-spin" /> : <Wallet className="w-4 h-4" />}
      Connect Wallet
    </button>
  );
};
    ...
    return paymentData.customFields.map((field, index) => {
      ...
      return (
        <div key={`${field.fieldName}-${index}`}>
          ...
          />
          {hasError && <p className="mt-1 text-sm text-red-500 font-figtree">{errors[field.fieldName]}</p>}
        </div>
      );
    });
  };
  ...
  const renderPaymentFlow = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Wallet Connection */}
        <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg dark:bg-gray-800/50">
        ...
        {isConnected && balance !== null && (
          <div className="p-4 bg-gray-50 rounded-lg dark:bg-gray-800/50">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-[#0e121b] dark:text-white">{balance.toFixed(2)} USDC</span>
            </div>
            {estimatedFee !== null && (
        ...
        {error && (
          <div className="p-4 bg-red-50 rounded-lg border border-red-200 dark:bg-red-900/20 dark:border-red-800">
            ...
            <button onClick={resetState} className="mt-3 text-sm text-red-600 dark:text-red-400 hover:underline">
              Try again
            </button>
          </div>
        )}
          ...
          {!isConnected ? (
            ...
            <>
              ...
              <button
                ...
                disabled={
                  !isConnected || status === "sending" || status === "confirming-tx" || status === "checking-balance"
                }
              ...
              >
                {(status === "sending" || status === "confirming-tx") && <FiLoader className="w-4 h-4 animate-spin" />}
      ...
      <div className="bg-white mt-40 dark:bg-[#0e121b] border border-[#E1E4EA] dark:border-[#2B303B] rounded-xl shadow-xl w-full max-w-[450px] p-6">
        {isLoading ? (
              ...
              <div className="flex gap-2 items-center">
                ...
                <div>
                  ...
                  <p className="text-[16px] text-[#525866] dark:text-[#99A0AE] tracking-text">
                    {showPaymentFlow ? "Complete your payment" : "Fill in these details to pay"}
                  </p>
                </div>
              </div>
              <div className="text-right">
          ...
          <div className="py-8 text-center">
            <p className="mb-2 text-lg text-red-500 font-figtree">Payment link not found</p>
            <p className="text-[#525866] dark:text-[#99A0AE]">This payment link may have expired or doesn't exist.</p>
          </div>
        )}
      </div>
    </div>
